...
# 區域與英文名稱、分類狀態
area   = ['北區'       , '東區'     , '中西區'      , '永康區']
label  = ['north.csv'  , 'east.csv' , 'midwset.csv' , 'yongkong.csv']
...
def get_total(name):
    ...
    for i in range(1,12):
        t = 107*100+i
        total_107  = total_list.append(len(df.loc[(df['交易年月日'] == t)].index))
    return total_list
...
def main():
    ...
    # 依區域分出告自CSV
    # for i in range(0,4):   
        # need_area_status(area[i], status[1], 'P_'+label[i])   #此區 "有車位"
        # need_area_status(area[i], status[2], 'N_'+label[i])   #此區 "無車位"  
    ...
    # for i in label:
        ...
        # plt.plot(index, b, label = i, marker='o')
        # for a, b in zip(index, b):
